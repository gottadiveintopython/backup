...
2. このスクリプト中の変数OUTPUT_DIRに抜き出した映像の出力先を指定
...
def extract_video(*, inpath, outpath):
    command = (
        'ffmpeg', 
        '-i', str(inpath),
        '-vcodec', 'copy', '-map',  '0:0',
...
def extract_all_video(dirpath, *, recursive):
    for path in find_entries(dirpath, recursive=recursive):
        print(path)
        extract_video(inpath=path, outpath=OUTPUT_DIR / path.name)


if __name__ == '__main__':
    extract_all_video(Path(), recursive=True)
